...
- Default pinned version: `v0.45.1`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.45.1` and exposes `packages.default`.
- **Stable tag (`v0.45.1`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.47.0-nightly.20260605.g4196596f7` and exposes `packages.nightly`.
