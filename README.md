...
- Default pinned version: `v0.46.0`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.46.0` and exposes `packages.default`.
- **Stable tag (`v0.46.0`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.48.0-nightly.20260613.g9e5599c32` and exposes `packages.nightly`.
