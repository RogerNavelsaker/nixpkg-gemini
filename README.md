...
- Default pinned version: `v0.42.0`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.42.0` and exposes `packages.default`.
- **Stable tag (`v0.42.0`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.44.0-nightly.20260518.g5611ff40e` and exposes `packages.nightly`.
