...
- Default pinned version: `v0.44.0`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.44.0` and exposes `packages.default`.
- **Stable tag (`v0.44.0`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.45.0-nightly.20260528.g5cac7c10f` and exposes `packages.nightly`.
