...
- Default pinned version: `v0.53.0`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.53.0` and exposes `packages.default`.
- **Stable tag (`v0.53.0`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.55.0-nightly.20260730.gdc859e8e4` and exposes `packages.nightly`.
