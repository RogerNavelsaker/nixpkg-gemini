...
- Default pinned version: `v0.56.0`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.56.0` and exposes `packages.default`.
- **Stable tag (`v0.56.0`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.56.0-nightly.20260825.g812f7a2bc` and exposes `packages.nightly`.
