...
- Default pinned version: `v0.60.0`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.60.0` and exposes `packages.default`.
- **Stable tag (`v0.60.0`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.62.0-nightly.20260920.gcfbcaa8df` and exposes `packages.nightly`.
