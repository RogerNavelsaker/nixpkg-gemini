...
- Default pinned version: `v0.43.0`
...
- **Release (default):** Uses `google-gemini/gemini-cli@v0.43.0` and exposes `packages.default`.
- **Stable tag (`v0.43.0`):** Uses the `gemini-cli-stable-src` input and exposes `packages.stable`.
- **Main:** Uses `google-gemini/gemini-cli@main` and exposes `packages.main`.
- **Nightly:** Uses `google-gemini/gemini-cli@v0.44.0-nightly.20260521.g57c42a5c4` and exposes `packages.nightly`.
